...
func NewCart() console.Cartridge {
	return &cartridge{
		BaseCartridge: console.NewBaseCart(console.ZXSpectrumConfig()),
	}
}

// Init - called once when cart is initialised
func (c *cartridge) Init() {
	c.ClsWithColor(console.ZX_WHITE)
...
func (c *cartridge) Render() {
	c.Cls()
	c.PrintAtWithColor(fmt.Sprintf("zx-spectrum:%d", c.counter), c.x, c.y, console.ZX_BLACK)
}
